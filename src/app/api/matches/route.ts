...
export async function POST(request: Request) {
  try {
    ...
    if (action === "create") {
      const { team_a_name, team_b_name, overs_limit, single_man, single_man_mode, toss_winner_id, toss_decision, bowler_overs_limit, series_id } = body;
      ...
      const result = await sql`
        INSERT INTO matches (
          ...
          single_man, single_man_mode, toss_winner_id, toss_decision, bowler_overs_limit, series_id
        )
        VALUES (
          ...
          ${toss_decision || null}, ${Number(bowler_overs_limit || 0)}, ${series_id ? Number(series_id) : null}
        )
        RETURNING id
      `;
        ...
        for (const stat of player_stats) {
          await sql`
            INSERT INTO match_player_stats (
              ...
              dot_balls_bowled, hatricks, wicket_how
            ) VALUES (
              ${Number(match_id)},
          ...
          await sql`
            ...
            SET matches_played = matches_played + 1,
                ...
                hatricks = hatricks + ${Number(stat.hatricks || 0)}
